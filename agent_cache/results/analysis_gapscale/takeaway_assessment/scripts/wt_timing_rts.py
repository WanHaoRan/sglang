...
fin = {}
for line in open(base + "/server.log", errors="ignore"):
    if "ReqTimeStats(rid=" in line and "-c" in line:
        mm = re.search(r"rid=\S+-c(\d+)-t(\d+),", line); m0 = ts_re.match(line)
        if mm and m0: fin[(int(mm.group(1)), int(mm.group(2)))] = ts(m0.group(1))
offs2 = [fin[(t["conv"], t["turn"])] - (t_start + t["t_send"] + t["latency"]) for t in turns if (t["conv"], t["turn"]) in fin]
off = st.median(offs2); print("finish-aligned offset", off, len(offs2))
